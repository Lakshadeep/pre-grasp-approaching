...
class Task(Environment):
    ...
    def __init__(
        ...
        Constructor.
        Args:
            max_dist (float, 0.05): max distance robot can travel between the single action step;
            max_base_turn (float, np.pi/24): max angle robot base can turn in between the single action step;
            horizon (int, 100): horizon of the problem;
            gamma (float, .95): discount factor.
        """
        # MDP properties

        # TODO: All these properties should go as constructor params eventually
        gamma = gamma                                
        horizon = horizon
        ...
        self._max_base_turn = max_base_turn
    ...
    def reset(self, state=None, robot_r_min=1.5, robot_r_max=3.0, table_r_max=0.4):
        ...
        if state is None:
            # NOTE: initial obj angle = 90, -180, 180, robot angle = 0, 0, -90 
            if self._random:
    ...
    def _set_up_scene(self) -> None:
        ...
        scene_path = "/home/sdur/Planning/Codes/motion-planning/isaac_envs/task.usd"
        ...
        self.lula_kinematics_solver = LulaKinematicsSolver(
            robot_description_path = '/home/sdur/Planning/Codes/motion-planning/ur5e_assets/robot_descriptor_basic.yaml',
            urdf_path = '/home/sdur/Planning/Codes/motion-planning/ur5e_assets/ur5e_gripper.urdf'
        )
        ...
        #             target_position=np.array([0.2,0,0.7]),
        ...
        #         # self._move_robot(0.05, 0.1, j4 = -37.82, j5 = 124)
        #         # self.reset()
                # time.sleep(0.1)
        #    time.sleep(0.1)
